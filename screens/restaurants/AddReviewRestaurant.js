...
export default function AddReviewRestaurant({ navigation, route}) {
    ...
    const addReview = async() => {
        ...
        const responseUpdateRestaurant = await updateDocumentById("restaurants", idRestaurant, {
            ratingTotal,
            quantityVoting,
            rating: ratingResult
        })
    ...
    const validForm = () => {
        ...
        if(isEmpty(title)){
            setErrorTitle("Debes ingresar un titulo a tu comentario.", 3000)
            isValid = false
        }

        if(isEmpty(review)){
            setErrorReview("Debes ingresar un comentario.", 3000)
            isValid = false
        }
